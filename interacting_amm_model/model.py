...
amms = {amm.label: amm for amm in amm_list}
# %%
# Simulation Initial State
initial_state = {
    # For this current iteration, price = token 2 price in terms of token 1
    'market_price': InitialValue(5, float),
...
params = {
    ...
    'user_action_intensity': ParamSweep([0.1, 0.2, 0.3], Percentage),
    'arbitrage_intensity': ParamSweep([0.01, 0.1, 0.3], Percentage),
    'swap_vs_liquidity_preference': ParamSweep([0.5, 0.7, 0.9], Percentage),
    'positive_direction_bias': ParamSweep([0.5], Percentage),
    'constant_market_price': Param(True, bool),
}
...
def p_arbitrage(params, _2, _3, state) -> Signal:
    ...
    for amm, pair_state in pair_states.items():
        ...
        optimal_arbitrage = PairState(-optimal_value,
                                      optimal_value * amm_price)
...
def p_user_action(params, _2, _3, state) -> Signal:
    ...
    """
    # Retrieve state and params
    positive_direction_bias = params['positive_direction_bias']
    ...
    for amm, pair_state in pair_states.items():
        ...
        if random() < positive_direction_bias:
            direction = 1
        else:
            direction = -1
...
def p_market_price(params, _2, _3, state) -> Signal:
    """
    Retrieve the market price as proxied by the mean value of the AMMs
    as well as its associated volatility
    """
    constant_market_price = params['constant_market_price']
    if constant_market_price is False:
        pair_states: AMM_States = state['pair_state']
        prices = [pair_state.reserve_token_2 / pair_state.reserve_token_1
                  for pair_state 
                  in pair_states.values()]
    ...
    return {'market_price': market_price,
            'market_price_volatility': market_price_volatility}
